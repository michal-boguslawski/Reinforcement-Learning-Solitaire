...
class Worker:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.env = make_vec(self.env_name, num_envs=num_envs, device=device)
    ...
    def _train_one_step(self):
        try:
            action_output = self.agent.action(method=self.action_exploration_method, state=self.state, epsilon_=self.epsilon)
    ...
    def _eval_record_video(self, num_step: int) -> None:
        env = make_env(
            ...
            device=self.device,
            record=True,
            video_folder = f"logs/{self.env_name}/videos/num_step_{num_step}",
        )
        total_reward = 0.
        step_count = 0
        state, _ = env.reset()
